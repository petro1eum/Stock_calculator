...
const SuppliesTab: React.FC = () => {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const dataStr = localStorage.getItem('wb_purchases');
      const allData = dataStr ? JSON.parse(dataStr) : [];
      const data = allData.filter((r: any) => {
        const d = new Date(r.date || r.date).getTime();
        return d >= from && d < to;
      });
      ...
      data.forEach((r: any) => {
        ...
        if (!map[day]) map[day] = [];
        const sku = String(r.nmId || r.sku);
        const name = nameBySku.get(sku) || r.subject || r.brand || null;
        let total = r.totalPrice !== undefined ? Number(r.totalPrice) : null;
        map[day].push({ date: day, sku, name, quantity: Number(r.quantity || 0), warehouse: r.warehouse || r.warehouseName || null, total_price: total });
      });
  ...
  const loadPurchaseOrders = React.useCallback(async () => {
    try {
      const ordersStr = localStorage.getItem('wb_purchase_orders');
      const itemsStr = localStorage.getItem('wb_purchase_order_items');
      const orders = ordersStr ? JSON.parse(ordersStr) : [];
      const items = itemsStr ? JSON.parse(itemsStr) : [];

      const ordersWithItems = orders.map((order: any) => ({
        ...order,
        items: items.filter((item: any) => item.po_id === order.id)
  ...
  const loadLogisticsEvents = React.useCallback(async () => {
    try {
      const eventsStr = localStorage.getItem('logistics_calendar');
      const events = eventsStr ? JSON.parse(eventsStr) : [];
      events.sort((a: any, b: any) => new Date(b.start_date).getTime() - new Date(a.start_date).getTime());
      setLogisticsEvents(events);
    } catch (e: any) {
      setError(e.message);
    }
  }, []);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const from = new Date(year, 0, 1).getTime();
        const to = new Date(year + 1, 0, 1).getTime();
        const costsStr = localStorage.getItem('wb_costs');
        const data = costsStr ? JSON.parse(costsStr) : [];
  ...
  }, [load, loadPurchaseOrders, loadLogisticsEvents, year]);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const pricesStr = localStorage.getItem('wb_prices');
        if (pricesStr) {
          JSON.parse(pricesStr).forEach((p: any) => {
            const sku = String(p.nm_id || p.nmId || p.nmID);
        ...
        if (anStr) {
          JSON.parse(anStr).forEach((a: any) => {
            const sku = String(a.nm_id || a.nmId || a.nmID);
            const nm = a.raw?.vendorCode || a.raw?.supplierArticle || a.raw?.name;
            if (nm && !map.has(sku)) map.set(sku, String(nm));
          });
        }

        const salesStr = localStorage.getItem('wb_sales');
        if (salesStr) {
          JSON.parse(salesStr).forEach((s: any) => {
            const sku = String(s.sku || s.nmId);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const purchasesStr = localStorage.getItem('wb_purchases');
        const data = purchasesStr ? JSON.parse(purchasesStr) : [];

        const map = new Map<string, { date: string; quantity: number; warehouse?: string | null }[]>();
        data.forEach((r: any) => {
          if (new Date(r.date || r.date).getTime() >= sinceMs) {
            const sku = String(r.sku || r.nmId);
  ...
  const savePurchaseOrder = async (orderData: Partial<PurchaseOrder>, items: Partial<PurchaseOrderItem>[]) => {
    try {
      ...
      let allItems = itemsStr ? JSON.parse(itemsStr) : [];
      let orderId = editingOrder ? editingOrder.id : String(Date.now());
      ...
      if (editingOrder) {
        const idx = orders.findIndex((o: any) => o.id === editingOrder.id);
        if (idx !== -1) orders[idx] = newOrder;
        allItems = allItems.filter((i: any) => i.po_id !== orderId);
      } else {
      ...
      localStorage.setItem('wb_purchase_orders', JSON.stringify(orders));
      localStorage.setItem('wb_purchase_order_items', JSON.stringify(allItems));
                      ...
                      <div className="flex gap-2">
                        <button className="text-xs px-2 py-1 border rounded hover:bg-gray-50" onClick={async () => {
                          try {
                            ...
                            const costsStr = localStorage.getItem('wb_costs');
                            let costs = costsStr ? JSON.parse(costsStr) : [];
                            const idx = costs.findIndex((c: any) => c.date === payload.date && c.sku === payload.sku);
                            if (idx !== -1) costs[idx] = { ...costs[idx], ...payload };
                            else costs.push(payload);
                        ...
                        <button className="text-xs px-2 py-1 border rounded text-red-600 hover:bg-red-50" onClick={async () => {
                          try {
                            ...
                            const targetDate = `${selectedDay}T00:00:00.000Z`;
                            const costsStr = localStorage.getItem('wb_costs');
                            let costs = costsStr ? JSON.parse(costsStr) : [];
                            costs = costs.filter((c: any) => !(c.date === targetDate && c.sku === r.sku));
                            localStorage.setItem('wb_costs', JSON.stringify(costs));
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    localStorage.setItem('logistics_calendar', JSON.stringify(events));
        ...
        <LogisticsEventForm
          ...
          onSave={async (payload) => {
            try {
              ...
              if (editingRisk) {
                const idx = events.findIndex((e: any) => e.id === editingRisk.id);
                if (idx !== -1) events[idx] = { ...events[idx], ...payload };
                toast.success('Событие обновлено');
              } else {
                events.push({ ...payload, id: String(Date.now()) });
