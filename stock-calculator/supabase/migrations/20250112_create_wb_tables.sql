...
-- USER SECRETS: store per-user WB API key
...
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='user_secrets' and policyname='user_secrets_select_own') then
    create policy user_secrets_select_own on public.user_secrets for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='user_secrets' and policyname='user_secrets_upsert_own') then
    create policy user_secrets_upsert_own on public.user_secrets for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='user_secrets' and policyname='user_secrets_update_own') then
