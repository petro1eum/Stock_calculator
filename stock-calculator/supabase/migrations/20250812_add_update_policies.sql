-- Add UPDATE RLS policies to allow upsert (conflict -> update) for own rows
...
do $$ begin
  if not exists (
    ...
    create policy wb_sales_update_own on public.wb_sales
...
do $$ begin
  if not exists (
    ...
    create policy wb_purchases_update_own on public.wb_purchases
      for update using (auth.uid() = user_id);
  end if;
end $$;
...
do $$ begin
  if not exists (
    ...
    create policy wb_stocks_update_own on public.wb_stocks
      for update using (auth.uid() = user_id);
  end if;
end $$;
